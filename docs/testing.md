...
| Test | Purpose |
|------|---------|
| `TestE2E_BusinessScenarios` | Validates all 4 scenarios (startup, enterprise, all-zombies, no-zombies) return expected cost and usage data |
| `TestE2E_DetectionRules` | Verifies analyzer correctly identifies zombies from fake provider data |
...
| `TestAllZombies_AllUsageIsZero` | "all-zombies" scenario has zero usage for all resources |
| `TestNoZombies_AllUsageAboveThreshold` | "no-zombies" scenario has usage above detection thresholds |
...
| `BenchmarkDetection` | Zombie detection algorithm (~1.7μs/op) |

**Scenarios:**
- `startup` — 2 accounts, mix of active and idle resources
- `enterprise` — 5 accounts, realistic production workload
- `all-zombies` — All resources idle (zero usage)
- `no-zombies` — All resources active (usage above thresholds)

Set `DEV_SCENARIO=all-zombies` in `services/ingestion/.env` to use a specific scenario.
...
| `TestZombies` | Zombie resources endpoint |
...
```bash
make seed              # 24 zombies, 36 resources, 90 days of realistic trend data
```
