...
Status: **near-term / scheduled** — ADR-0002 (SaaS-first for awareness)
...
3. **Entitlement bootstrap (§7.1)** — **scaffold now, real `entitlements` before charging.** Use the one-line platform-license scaffold for the free validation beta; build the real per-tenant `entitlements` table + bypass wiring only once self-serve activation proves out (the ADR-0002 validation gate). No billing/entitlement plumbing before the PLG bet is validated.
