...
> **Status (2026-06-10):** the per-tenant `entitlements` table (migration 033) + the `entitlement` package + the **bypass wiring** have shipped ahead of the activation gate, at the founder's explicit direction. The wiring is gated behind a `saashosted` **build tag** (`services/{api,ingestion}/cmd/saasmode_*.go`) — `go build -tags "production saashosted"` produces SaaS binaries that call `license.SetEnforcementBypass()` at boot and gate scans on entitlement; the default self-hosted build is unchanged and the bypass is not compiled into it. Billing/Stripe is still unbuilt — entitlement rows are written by `cmd/entitlement-seed` (or the CI seed step below) only. So the *mechanism* exists ahead of the gate, but it ships nothing to a **customer** until a real SaaS deploy + billing land. The platform-license scaffold (the option above) remains the path for any pre-billing beta.
>
> **Update (2026-06-11):** the first SaaS deploy target now exists — **dev-1**, as an internal preview. `build:images-saashosted` builds api+ingestion with `-tags saashosted` *alone* (no `production` → DEV_MODE/no-auth preserved) and `deploy:dev-1` pulls that variant + seeds the dev org's entitlement row. This is a dev-preview, **not** a customer-facing deploy (that needs the canonical `production saashosted` shape + real auth + billing). See [`saas-dev1-deploy-target.md`](saas-dev1-deploy-target.md).
>
> *Implementation note: the original plan named separate `cmd/api-saashosted` / `cmd/ingestion-saashosted` composition roots. That doesn't work for ingestion (its wiring lives in `package main`, which a sibling `main` package can't import), so the implementation uses a `saashosted` build tag on the existing roots instead — same compile-time guarantee, no duplicated `main`.*
