...
- Five composition seams are preserved for reactivation: `auth.Provider`, `auth.Inviter`, `serverbuild.ComposeServer`, `sso.Discoverer`, `sso.Connector`. The license package additionally exposes `SetEnforcementBypass()` / `IsEnforcementBypassed()`, reserved for `cmd/api-saashosted`.
