...
| `services/api/cmd/license-issue/main.go` (slice 7 — not yet shipped) | Operator-side CLI for issuing licenses. Reads signing key from `$LICENSE_SIGNING_KEY_PATH`, prompts for customer_id + contract_id + duration, outputs the JWT. Documented in `docs/license-issuance.md`. |
| `services/api/internal/api/handler.go` | `triggerScan` consults `license.IsScanAllowed()` and 403s with `license_expired` past grace (§4.9.2b). **Slice 6 (not yet shipped)** will extend `getVersion` to return `license: {customer_id, expires_at, days_remaining, state, max_organizations}`. |
| `services/dashboard/src/components/LicenseBanner.jsx` (slice 8 — not yet shipped) | Top-of-page banner shown when license is in grace period or has <14 days remaining. Owners only. |
| `services/dashboard/src/screens/AccountsScreen.jsx` (modify, slice 8) | On 403 `license_expired` from the scan button, surface a toast with the renewal contact. Button stays clickable so the user gets a clear message rather than a mystery-disabled control. |
