...
**Files**
- `services/shared/storage/postgres/migrations/NNN_stripe_billing_wiring.up.sql` / `.down.sql` — **new** (dedupe table + the `entitlements` `price_id`/index additions; see the naming note below). `NNN` is the next free number at implementation time (`035` today, but the companion plan also claims it — do NOT copy `035` verbatim; `ls migrations/ | tail` first).
...
**Shared-struct addition (DS9).** `entitlement.BillingEvent` gains a `PriceID string` field; `model.Entitlement` gains a matching `PriceID string`; `ApplyBillingEvent` (`services/shared/entitlement/billing.go`) sets `ent.PriceID = evt.PriceID`. This is the one cross-cutting touch beyond the api `billing` package, and it is **clean, not high-churn** — one shared `entitlementSelectSQL` const + one `scanEntitlement` helper + one `UpsertEntitlement` impl in `storage/postgres/entitlement.go`. Add `price_id` to the const (SELECT clause) and the scan helper, and add the column + `EXCLUDED.price_id` to the single INSERT/UPSERT statement. The `Writer.UpsertEntitlement(ctx, model.Entitlement)` **interface signature is unchanged** — it continues to take the full model by value; only the implementation route updates. **Follow the existing `*string` empty↔NULL mapping** the billing refs use (entitlement.go ~lines 85–91): write `''` as SQL NULL and `COALESCE(price_id, '')` on read, so trial/`internal` rows store NULL — which is exactly the signal Slice 8's `plan_display` fallback ("Trial"/"Internal") keys on. This direct lookup replaces a fragile reverse-map.
...
- `services/api/internal/billing/checkout.go` / `portal.go` — these DO have a session actor (owner), so they use the normal `audit.Record(r, w, e)` helper (signature `Record(r *http.Request, w audit.Writer, e model.AuditEvent)` — the 2nd arg is the `audit.Writer` seam the store satisfies, not the full `Store`).
...
7. **The audit helper `Record(r *http.Request, w audit.Writer, e model.AuditEvent)` enriches from the *request*** (`services/api/internal/audit/audit.go` line 39 — org id + user id from request context); the webhook has neither, so Slice 9 writes via the lower-level `store.AuditLogWrite` with an explicit org + system actor — the first real caller of the "non-member actor" gap noted in `saas-platform-admin-design.md` §8.
