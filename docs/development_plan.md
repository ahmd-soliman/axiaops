...
**Priority: Before production deployment — the orchestrator (ECS Express Fargate) sends `SIGTERM` before terminating containers.**
...
- **deploy:** task-definition update + `aws ecs update-service` for `api` and `ingestion` (ECS Express); CloudFront invalidation for dashboard
...
- **Infrastructure-as-code:** Docker Compose for dev; production infra defined in Terraform (ECS Express, RDS, Secrets Manager) — reproducible from scratch. State backend: S3 bucket + DynamoDB lock table. Lives in the sibling [`axiaops/aws-infra`](https://gitlab.com/axiaops/aws-infra) repo.
...
- API service: AWS ECS Express (Fargate, always-on behind an ALB) — see `docs/deployment.md`
- Ingestion service: ECS Express (long-lived Fargate task, receives HTTP scan requests)
...
| Hosting | AWS ECS Express (Fargate behind ALB) |
