...
For ephemeral demo envs (the public demo deployment scenario in `docs/feature_prioritization_2026_q2.md`), the bootstrap ceremony is friction. The `--bootstrap-first` flag skips it by minting the first org + alice as the first owner directly via SQL, then sealing `/auth/bootstrap` so subsequent install-token POSTs return 409.
...
- `docs/feature_prioritization_2026_q2.md` §D — the public demo deployment scenario `--bootstrap-first` was designed for.
