...
**Sequencing vs the pre-billing security pass and billing.** The expected order is **signup beta (this plan, ADR-0002's "first build") → pre-billing security pass ([`security-before-billing-plan.md`](security-before-billing-plan.md) §2) → Stripe build ([`billing-plan.md`](billing-plan.md))**. Two of the pass's items intersect this plan and are cross-referenced where they bite: **H-1** (users-table RLS) **has already landed** (migration 035) — `RegisterSelfService`'s admin-pool reuse stays valid because it runs on `s.adminPool`, covered by `users_runtime_bypass` (see Slice 1's RLS note + grounding note 1); and **M-8** (CSRF) makes register an additional CSRF-cookie-issuance site (see Slice 2), still open. Re-validate the CSRF intersection before coding Slice 2.
...
**RLS note.** Uses `s.adminPool` exactly as `ConsumeBootstrapState` (registration is pre-org-context — it *creates* the org). The membership INSERT sets `app.organization_id` via `set_config(..., true)` in-tx. `organizations`/`sessions` have no RLS; `memberships` does and is satisfied by the local GUC. **H-1 has landed (migration 035): `users` is now RLS-scoped, and the `users` INSERT relies on the `users_runtime_bypass` policy that covers the `axiaops_runtime` role — so the admin-pool transaction reuse is valid as long as `RegisterSelfService` runs on `s.adminPool` (it does). Do NOT move the `users` INSERT to the app pool; with no org GUC set the `users_organization_isolation` WITH CHECK would reject it.**
...
`NNN` is the next free number at implementation time — **036** today (`035_users_rls` landed the H-1 users RLS migration; the companion [`billing-plan.md`](billing-plan.md) Slice 1 also drafted against `035`, so **do NOT copy a number verbatim** — `ls migrations/ | tail` and take the next free; whichever plan lands second renumbers). `users` is now **RLS-scoped** (migration 035) with a **global** `lower(email)` unique index (migration 021). Verification state still lives on `users`, but any pre-auth read/write of it must go through the runtime-bypass pool (`s.adminPool`) — the `users_organization_isolation` policy filters the app pool. The token belongs in a sibling capability table; scope it like `sessions` (no RLS, runtime-pool access) rather than a per-org RLS table, since email verification is pre-org-context.
...
- `model.User` gains `EmailVerifiedAt *time.Time` — **propagate the new column to every `SELECT … FROM users` that scans into `model.User`** (`CreateUserWithPassword`, `LookupUserByEmail`, `LookupInvitationByToken`, `RedeemNativeInvitation`, `ConsumeBootstrapState`) or the scans break. Highest-churn risk in this slice; enumerate them. **Sequencing note:** the H-1 users-RLS work (migration 035) already touched the *same family* of users-scan sites (`GetUserByID`/`UpsertUser`/`GetUserByEmail`/`SetUserSSOConnection`/`GetUserSSOConnectionID` now read on `s.adminPool`; `EnsureUser` on an org-scoped admin-pool tx). When adding `EmailVerifiedAt`, update those scans on whichever pool they now use — and remember the rule established by H-1: new pre-auth `users` access goes through the runtime pool, never the app pool.
...
1. `users` is **RLS-scoped** (migration 035, H-1) and has a **global** `lower(email)` unique index (`021_native_auth.up.sql` line 36, `CREATE UNIQUE INDEX users_email_lower_unique`). The unique index is why `RegisterSelfService` reuses the bootstrap admin-pool transaction wholesale and `23505 → ErrUserEmailExists` is the concurrent-same-email guarantee — **unaffected by H-1**. **H-1 resolution:** migration 035 added `users_organization_isolation` (app pool, org-scoped) + `users_runtime_bypass` (covers `axiaops_runtime` SELECT/INSERT/etc.). `RegisterSelfService` runs on `s.adminPool` → the bypass policy covers its `users` INSERT, so the admin-pool reuse stays valid. The app-pool callsites the security plan named (`GetUserByID`/`UpsertUser`/`GetUserByEmail`/`SetUserSSOConnection`/`GetUserSSOConnectionID`) were moved to the runtime pool, and `EnsureUser` to an org-scoped admin-pool tx; the lesson for new pre-auth `users` access is **runtime pool, never app pool**.
