...
| `applied_by_actor` | `TEXT` | YES | NULL | "Who/what applied this." Resolution chain: `MIGRATION_ACTOR_LABEL` env var → `os.Hostname()` → literal `"unknown"`. CI deploys (`.gitlab-ci.yml` `.deploy-dev` / `deploy:staging`) set the env var to `ci:${CI_PROJECT_PATH}#${CI_PIPELINE_ID}:${GITLAB_USER_LOGIN}` so each row records the pipeline that applied it and which operator clicked the manual deploy gate. Without the override, falls through to the migrate container's auto-assigned 12-char Docker hostname (e.g. `b15f7ba7da2f`) — useless for forensics. Manual emergency invocations (operator SSH'd into a host) should set `MIGRATION_ACTOR_LABEL=manual:<incident-id>:<your-name>` for clarity; the hostname fallback is acceptable but ambiguous. Free-form text on purpose — operators want to grep, not enforce a schema. |
