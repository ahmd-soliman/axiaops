...
func TestRuntimeAdmin_BypassesRLSAcrossOrgs(t *testing.T) {
	...
	ctx2, org2 := newOrgCtx(t, s)
	mustSaveTestAccount(t, s, ctx1, org1.ID, "test-account-1")
	mustSaveTestAccount(t, s, ctx2, org2.ID, "test-account-2")
	if _, _, err := s.Save(ctx1, []model.CostRecord{costRecordFor("test-account-1", "AmazonEC2", "eu-central-1", 100)}); err != nil {
		t.Fatalf("seed org1: %v", err)
	}
	if _, _, err := s.Save(ctx2, []model.CostRecord{costRecordFor("test-account-2", "AmazonRDS", "eu-central-1", 200)}); err != nil {
