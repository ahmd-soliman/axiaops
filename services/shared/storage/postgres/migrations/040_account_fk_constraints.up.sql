...
--
-- ADD CONSTRAINT has no IF NOT EXISTS form in Postgres, so each is wrapped in
-- a DO block catching duplicate_object -- this migration must tolerate being
-- replayed by dirty-state recovery after a mid-step crash (see
-- migration_history.go's reapplyDirty), same pattern migration 031 uses for
-- policies.
