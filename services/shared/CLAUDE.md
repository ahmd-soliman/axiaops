...
| `license/` | **Phase B1.6 (amended — see `docs/b1.6-amendment-feature-gating.md`) + B1.7 layer 4 (issue #75 — see plan §4.10.8)** — self-hosted license JWT verification. Embedded production RS256 public key (`pubkey.pem`); dev-only RS256 public key + 100-year fixture JWT in default builds (`pubkey-dev.pem` / `fixture-dev.jwt`, stripped from `-tags production` builds via `embed_{dev,production}.go`); `Load`, `CheckExpiry`, `VerifyAtBoot`, `RunTicker`, `IsScanAllowed`, `IsEnforcementBypassed`, `SetEnforcementBypass`. Both api and ingestion call `VerifyAtBoot` at startup (always returns nil now — never refuses to start except for the layer-2 anti-tamper case) and run `RunTicker`. **This package is the OPT-IN gate now: it governs scans only in the `-tags selfhosted` build.** The default (SaaS) build calls `SetEnforcementBypass()` at boot (via `cmd/saasmode_saas.go`), so `IsScanAllowed` returns true regardless of license state and the `entitlement/` package owns the scan decision. In `-tags selfhosted`, DEV_MODE loads the embedded fixture (state=Valid) and `IsScanAllowed` returns true when state ∈ {Valid, InGrace}; `StateExpired` / `StateNotLoaded` gate. `IsEnforcementBypassed()` / `SetEnforcementBypass()` are flipped by the default SaaS build seam (`cmd/saasmode_saas.go`); the licensed self-hosted build never sets the bypass. |
| `entitlement/` | **SaaS per-tenant entitlement (see `docs/saas-platform-admin-design.md` §7.2 + ADR-0002).** The SaaS analogue of `license/`: answers "may this org run a paid scan?" from billing-driven entitlement state instead of a license JWT. `IsScanAllowed` (pure predicate — trialing/active allowed, past_due allowed inside `CurrentPeriodEnd + grace`, canceled/suspended gated, unknown gated), `Resolver` (read seam, satisfied by `storage.Store`), `IsScanAllowedForOrg` (wrapper the scan gates call — **fail-closed**: missing row → deny-no-error, DB error → deny-and-surface), and the billing seam stub (`BillingEvent` + `ApplyBillingEvent` — provider-agnostic projection; real Stripe parsing is out of scope). **This is the DEFAULT scan gate now.** The default (SaaS) build calls `license.SetEnforcementBypass()` at boot (license dormant) and threads the store as the Resolver into the api `scanAccount` + the 3 ingestion gates; only the `-tags selfhosted` opt-in build passes a nil resolver and falls back to the license gate, leaving this package inert there. Build-tag seam: `services/{api,ingestion}/cmd/saasmode_*.go`. **Auto-entitlement:** every org gets a default `active`/`internal` row at creation time (idempotent, `adminPool`, `ON CONFLICT DO NOTHING`) so the fail-closed gate passes pre-billing; `cmd/entitlement-seed` is the manual override and a future billing webhook overwrites via `ApplyBillingEvent`. |
...
- Tables: `organizations`, `users`, `memberships`, `pending_memberships` (email-based invitations awaiting first-login redemption — see `docs/invitation-flow.md`),
  ...
  `entitlements` (SaaS per-tenant plan/status/limits — one row per org; **system-scoped, no RLS**, granted to `axiaops_runtime` only; migration 033, `'internal'` plan + org backfill in 034; **the active default scan gate** — every org is auto-entitled at creation, see `entitlement/` above and `docs/saas-platform-admin-design.md` §7.2)
