...
| `entitlement/` | **SaaS per-tenant entitlement — DORMANT Phase 2A scaffold (see `docs/saas-platform-admin-design.md` §7.2 + ADR-0002).** The SaaS analogue of `license/`: answers "may this org run a paid scan?" from billing-driven entitlement state instead of a license JWT. `IsScanAllowed` (pure predicate — trialing/active allowed, past_due allowed inside `CurrentPeriodEnd + grace`, canceled/suspended gated, unknown gated), `Resolver` (read seam, satisfied by `storage.Store`), `IsScanAllowedForOrg` (wrapper the future scan gates call — **fail-closed**: missing row → deny-no-error, DB error → deny-and-surface), and the billing seam stub (`BillingEvent` + `ApplyBillingEvent` — provider-agnostic projection; real Stripe parsing is out of scope). **No scan gate consults this yet** — the `*-saashosted` composition roots that call `license.SetEnforcementBypass()` and wire the gates are Phase 2B, deferred until ADR-0002 is accepted (§7.1). Writes go through `cmd/entitlement-seed` (manual/dev) today. |
