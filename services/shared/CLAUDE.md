...
|---------|---------------|
| `model/` | Domain types: Organization, User, Account, CostRecord, ZombieResource, ResourceRecord, ZombieSnapshot, SnapshotService, NotificationChannel/NotificationDispatch/TriggerRule + EmailConfig/SlackConfig/TeamsConfig (`notification.go`) |
...
- Three DB roles: `axiaops_owner` (runs migrations, creates schema — migrate task only), `axiaops_runtime` (least-privilege RLS-bypass via per-table policies, no DDL — the runtime services' `adminPool`; see `docs/AUTHENTICATION.md` § 5), and `axiaops` (app user, RLS-limited)
