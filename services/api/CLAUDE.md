...
Test pairs in `cmd/devmode_{dev,production}_test.go` + `cmd/saasmode_{saas,selfhosted}_test.go` regression-pin the shapes; CI runs `make build-production` (SaaS) on every pipeline and `build:selfhosted-shape` pins the `production selfhosted` shape, so tag regressions surface in <30s.
