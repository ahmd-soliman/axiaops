...
| GET | /v1/version | Yes | Build identifier — `{service, version, commit, env}`. |
| GET | /v1/zombies | Yes | List zombie resources for organization |
| GET | /v1/summary | Yes | Aggregate savings + per-service breakdown (?account_id to scope to one account) |
| GET | /v1/summary/by-account | Yes | Per-account waste rollup for the org dashboard (`{currency, accounts:[{internal_account_id, account_id, total_zombies, potential_monthly_savings, top_service}]}`); zero-zombie accounts omitted; dismissed excluded like /summary |
| GET | /v1/trend | Yes | Zombie snapshots over time (?account_id, ?service, ?resource_type) |
| GET | /v1/trend/services | Yes | Distinct services available in trend data |
| GET | /v1/trend/resource-types | Yes | Distinct resource types for a service (?service) |
| GET | /v1/costs | Yes | Cost records for the organization (`?account_id`, `?service`, `?days`, default 30) |
| GET | /v1/resources | Yes | All resource records (zombies + active) |
| GET | /v1/accounts | Yes | List connected cloud accounts |
| GET | /v1/accounts/{id} | Yes | Get a single connected account |
| POST | /v1/accounts | Yes | Connect new account (encrypts secret) |
| POST | /v1/accounts/draft | Yes | Kick off role-based onboarding — body `{label, region}`; generates a server-side ExternalId, persists a `pending_role_setup` row, returns it (external_id JSON-visible so the dashboard can render the trust policy template). See `docs/OPERATIONS.md` § 1. |
| PATCH | /v1/accounts/{id} | Yes | Update label, region, secret_key, scan_interval_hours |
| DELETE | /v1/accounts/{id} | Yes | Remove account |
| GET | /v1/accounts/{id}/cur-setup | Yes | CloudFormation template body for CUR/Athena delivery setup. `?cur_database`/`?cur_table`/`?cur_workgroup` flow the dashboard's Advanced Configuration overrides into the template's Glue/Athena resource names; validated with the same regexes `updateAccount` uses (`validateCURConfig`) since values are interpolated into raw YAML unescaped. |
| GET | /v1/scan-permissions | Yes | The read-only IAM policy document for manual (access-key) onboarding — single source of truth shared with the CFN-managed role so they can't drift. `?billing_source=cur_athena` adds the Athena/Glue statement. |
| POST | /v1/accounts/{id}/scan | Yes | Trigger on-demand ingestion scan |
| GET | /v1/channels | Yes | List notification channels (`channels:read`). `config` returns secret fields (`smtp_pass`/`webhook_url`) masked as `***`. |
| POST | /v1/channels | Yes | Create a channel (`channels:manage`). Body `{kind, label, enabled, trigger_rule, config}`. v1 kinds: `email`, `slack`, `teams` (jira pre-provisioned in the DB enum, rejected here). Config encrypted at rest. Defaults `enabled:false`. |
| PATCH | /v1/channels/{id} | Yes | Update label/enabled/trigger_rule/config (`channels:manage`). `kind` is immutable. A secret field sent as `***` or empty keeps the stored value; any other value re-encrypts. |
| DELETE | /v1/channels/{id} | Yes | Delete a channel and its dispatch rows (`channels:manage`). |
| POST | /v1/channels/{id}/test | Yes | Send a fixed synthetic digest via the channel's transport (`channels:manage`). 200 `{status:"sent"\|"failed", error?}` — the HTTP call succeeds even when delivery fails; records a `notification_dispatches` row. |
| GET | /v1/channels/{id}/dispatches | Yes | Recent delivery attempts for a channel (`channels:read`, `?limit`, default 50 / max 200). |
| POST | /v1/dismissals | Yes | Dismiss or snooze a zombie resource |
| GET | /v1/dismissals | Yes | List active dismissals (?account_id) |
| DELETE | /v1/dismissals/{id} | Yes | Revoke a dismissal |
| GET | /v1/audit | Yes | Organization audit timeline (?user_id, ?resource_type, ?resource_id, ?action, ?since, ?until, ?limit, ?cursor) |
| GET | /v1/me | Yes | Current user's role + permission set + display `name` (always present, empty string when unset) + memberships + auth provider/mode. No permission required beyond authn. Display-name editing tracked under issue #78. |
| GET | /v1/memberships | Yes | List organization memberships |
| POST | /v1/memberships | Yes | Promote an existing user (by user_id) and assign a role; admin+ only |
| POST | /v1/invitations | Yes | Invite by email. Writes a token-bearing `pending_memberships` row and returns `redemption_url` in the response body for OOB sharing (admin pastes into Slack/email). Also **best-effort emails the redemption URL** to the invitee via the `InviteMailer` seam (per-org email channel first, then the global `SMTP_*` config) — outcome reported in `email_delivery` (`sent` \| `failed` \| `skipped_no_transport` \| `skipped_no_public_host` \| `error`) and metered via `axiaops_auth_invite_email_total{outcome,source}`; never fatal, the URL is always returned. Admin+ for member/viewer; owner for admin. Optional `enforcement_hint: "sso_required"` is set when the org has at least one active OIDC connection with `enforcement="required"` — the URL still works for the redemption hop but every authed request afterward 403s with `sso_required`, so the dashboard renders a yellow callout telling the admin to route the invitee through SSO instead. |
| GET | /v1/invitations | Yes | List pending invitations for the current org (?status=pending\|expired\|revoked) |
| DELETE | /v1/invitations/{id} | Yes | Revoke a pending invitation. The OOB redemption URL becomes invalid the moment the row flips to revoked. |
| PATCH | /v1/memberships/{id}/role | Yes | Promote or demote a member; permission tier depends on target role |
| DELETE | /v1/memberships/{id} | Yes | Remove a member; self-leave bypasses permission check (last-owner guard still applies) |
| POST | /v1/organizations/transfer-ownership | Yes | Atomically transfer owner role to another user; owner only |
| PATCH | /v1/organizations/me | Yes | Rename the current organization. Local DB rename + audit. Owner-only (`organization:update`). |
| POST | /v1/organizations/me/onboarding/complete | Yes | Mark onboarding complete for the org. Idempotent — already-complete returns the existing timestamp. |
| DELETE | /v1/users/me | Yes | Right-to-erasure: hard-delete the caller. 409 if sole owner of any organization — must transfer or delete those organizations first. Anonymises audit_log across all organizations. |
| DELETE | /v1/organizations/me | Yes | Right-to-erasure: cascade-delete the entire organization (every per-organization table including audit_log). Owner-only (`organization:delete`). |
| GET | /v1/export | Yes | JSON export of the organization's data (accounts, costs, zombies, audit log) — `data:export` permission. |
| POST | /v1/users/{id}/password-reset | Yes | Mint an admin-issued password-reset token. Returns `{user_id, redemption_url, expires_at}` for OOB sharing. Admin+ via `members:manage_basic`; owners-only when target is owner (cross-tier escalation guard). TTL via `PASSWORD_RESET_TTL_HOURS` (default 4h). |
| GET | /v1/auth/bootstrap/state | No | First-run probe. Returns `{available: bool}` — true iff a `bootstrap_state` row exists (i.e. a POST to `/auth/bootstrap` would succeed with the right token). Drives the dashboard's mount-time auto-redirect from `/login` → `/bootstrap` on fresh installs. Not a new oracle — same posture is observable today by POSTing junk to `/auth/bootstrap` and reading 409 vs 401. |
| POST | /v1/auth/bootstrap | No | First-owner install. Body `{token, email, name, password, organization_name}`. Sealed forever after first success — returns 409 on subsequent attempts. Token comes from `BOOTSTRAP_TOKEN_FILE_PATH` (mode 0600) or `BOOTSTRAP_INSTALL_TOKEN` env. |
| POST | /v1/auth/login | No | Native email + password. Single-membership users: returns 200 with `{user, organization}` and sets the `axiaops_session` cookie. Multi-membership users (B1.5): returns 200 with `{needs_org_selection:true, orgs:[{id,name},...]}` and **no cookie** — the dashboard collects the chosen org and POSTs it to `/auth/select-org`. Rate-limited 10/min/IP + 5/min/email. |
| POST | /v1/auth/select-org | No | B1.5 picker step. Body `{email, password, organization_id}`. Re-validates the password from scratch (defence in depth — never trust the frontend to remember step 1), confirms the chosen org is in the user's membership set, mints a session bound to it. Failure modes collapse to one 401 `invalid_credentials` shape (wrong password, unknown email, org-not-in-set all look the same — narrows the no-creds membership-probe channel). Shares the rate-limit budget with `/auth/login` so an attacker can't double their cap by alternating endpoints. |
| POST | /v1/auth/switch-org | (cookie) | B1.5 in-app org switcher. Body `{organization_id}`. Session-authenticated — caller is already logged in and just re-binds to a different org they're a member of. Same-org POST is a 200 no-op (idempotent for racy clients). Wrong org → 403 `not_a_member` (NOT 401-collapsed: the caller's session is valid; "not a member" is a separate posture from "wrong creds"). On success: revokes the current session (PG + cache + `axiaops_session_revocations_total{reason="org_switch"}`), mints a new one bound to target, sets fresh cookie, audits `session.org_switched` in the FROM org with metadata `{from, to}`. |
| POST | /v1/auth/logout | (cookie) | Revoke the current session and clear the cookie. Tolerant — 204 even when cookie is absent or unknown. |
| POST | /v1/auth/invitations/preview | No | B1.5 peek-before-redeem. Body `{token}`. Returns `{email, organization_name, role, existing_user}` so the AcceptInviteScreen can pick the right form mode (set new password vs verify existing password). Token is NOT consumed. 410 on unknown / expired / already-redeemed. Neither the user's password hash nor display name are exposed; audit M-9 prevents cross-org name disclosure via token enumeration. |
| POST | /v1/auth/invitations/redeem | No | Accept an invite token. Two flows the server selects on by looking up the email globally: **new user** — body `{token, password, name}`; server hashes, creates the user, inserts the membership. **Existing user (B1.5 cross-org)** — body `{token, password}`; server verifies the supplied password against the user's stored argon2id hash and only inserts the membership (the user row stays untouched). Both flows mint a session bound to the new org. 410 on invalid token, 401 invalid_credentials on wrong password (existing-user flow), 409 email_taken on race. |
| POST | /v1/auth/password-reset/redeem | No | Set a new password from an admin-issued reset URL. Body `{token, new_password}`. Revokes EVERY live session for that user on success — frontend must redirect to `/login`. 410 on unknown / expired tokens. |
...
| REDIS_URL | No | — | Connection URL for the cache + rate-limit counters + api→ingestion scan-enqueue backend (see `services/shared/{cache,queue}`). Empty ⇒ in-memory cache (single-replica only, see `docs/guides/aws-deployment` on the website), rate limiting disabled, and scan triggers fall back to a synchronous HTTP POST to `INGESTION_URL`. |
| INGESTION_URL | No | http://localhost:8081 | Where `api` POSTs on-demand scan triggers and role-verification calls when there's no queue (or as the queue's sync-mode target). Tests override via `WithIngestionURL`. |
...
| AXIAOPS_AWS_ACCOUNT_ID | No | — | AxiaOps' own AWS account ID, interpolated into the CUR-setup CloudFormation template's trust policy (`GET /v1/accounts/{id}/cur-setup`) so a customer's stack trusts the right account. |
...
| DEV_USER_NAME | No | Dev User | Display name for the dev user row |
