...
| 2.7.22 | **[POST-B2] Drop connection ID from OIDC callback path** | 🔲 | Surfaced during the AxiaOps Inc Entra walkthrough on 2026-05-07. Today the OIDC callback URL is `/v1/sso/oidc/{cid}/callback` — non-standard shape that forces every customer to register a connection-specific redirect URI in their IdP (Entra/Okta/Keycloak) and re-register if they ever delete + recreate the connection. Most OIDC RPs (Auth0, Kinde, Clerk, every standard library example) use a single stable callback URL like `/auth/callback` and resolve the connection from the `state` parameter, which is already the round-trip binding. **Shape**: (a) refactor route registration in `services/api/internal/serverbuild/build.go` and `cmd/main.go` to register `/v1/sso/oidc/callback` (no `{cid}`) alongside the existing path-cid form; (b) update `redirect_uri` builder in `services/api/internal/sso/initiate.go` to omit cid; (c) at callback, `cid` is already in `StateData.CID` from `state.Consume`, so handler reads it from there instead of `r.PathValue("cid")`; (d) keep the path-cid route registered for one release as a deprecation window so already-registered IdP redirect URIs still work; emit `axiaops_sso_legacy_callback_total{cid}` counter to detect lingering use; (e) remove path-cid route in the release after that. **Trade-off**: small defense-in-depth loss — today the path-cid + state-cid match check at callback would catch a hypothetical state-store compromise where an attacker rewrites a state record's CID (e.g. Redis bug). The state value is a 256-bit CSPRNG nonce (`state.go:82` `randomBase64URL(32)`, no HMAC) stored under `sso:state:{state}` with a 10min single-use TTL — an attacker who can write arbitrary keys to the cache could insert a fake record regardless, so the path-cid check is more "second seam against an internal bug" than crypto-grade defense. The standard shape is worth the swap. **Tests to update**: callback unit tests (`services/api/internal/sso/oidc_callback_test.go`), OIDC integration test (`services/api/internal/sso/oidc_integration_test.go`); only the happy-path `TestSSO_OpenRedirect_HappyPath` in `redirect_fuzz_test.go` needs touching (Layer 1 + Layer 2 fuzz tests are path-shape independent). **Why POST-B2 not Phase E**: pure DevX cleanup, not a feature; small enough to pair with the `docs/sso-local-entra.md` doc that came out of the same walkthrough. **Effort**: S (~½d code + ~10–15 test updates + 1 release-deprecation gap). |
