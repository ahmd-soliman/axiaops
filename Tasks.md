...
- [ ] **Secrets Manager**
  - [ ] Store `ENCRYPTION_KEY`, `REDIS_URL`, `RESEND_API_KEY`, `LICENSE_SIGNING_KEY` (issuer side)
...
| 2.7.12 | **[POST-B1] Re-evaluate DEV_MODE auth bypass** | 🔲 | DEV_MODE was introduced because Kinde JWT validation needed network access to a Kinde tenant — unworkable for local dev. With native auth (kinde now removed), that justification is gone: bootstrap is one form submission against a fresh DB, no external service. **Decision (not work)**: weigh three options against actual operator feedback: (a) **drop entirely** — operators run bootstrap once on fresh-DB dev like everyone else; pro: -30 LoC of auth-bypass branch + removes the "DEV_MODE accidentally set in prod" attack surface; con: every DB wipe costs one extra step. (b) **Replace with `make seed-bootstrap`** — make target that runs bootstrap via curl against the API; pro: removes auth-bypass code path entirely; keeps one-command dev iteration; con: makefile coupling, the curl call needs to know the install token path. (c) **Auto-bootstrap on fresh DB when `DEV_USER_EMAIL` is set** — server detects no-orgs-AND-dev-env and bootstraps itself; pro: keeps current one-step ergonomics; con: keeps an auth-bypass code path, just relabelled. Effort once decided: S (≤1d for any of the three). |
