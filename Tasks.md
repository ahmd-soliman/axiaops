...
| 2.7.5 | **License / entitlement model** | 🔲 | _Deferred until ≥3 paying customers._ For first 5 design partners, annual contracts + good faith are sufficient. Then design license-key mechanism (signed JWT containing `org_id, valid_until, max_accounts, features`) checked at app start. Effort: M (2w) **when** triggered. |
...
| 2.7.15 | **[partial — extend] Operator smoke-test runbook** | 🟡 | Plan §4.6 AC1, AC4. **Done in B1**: `docs/native-auth-bootstrap.md` covers the local `make start-staging` flow end-to-end — mkcert setup, install-token retrieval (with the zsh-`%`-not-part-of-token gotcha), bootstrap form, post-bootstrap verification, and a troubleshooting section for the common failure modes. **Still to do post-B1**: (a) the same walkthrough but for the legacy `AUTH_PROVIDER=kinde` path during the strangler window, (b) member invite / password-reset round-trip walkthroughs, (c) the install path for self-hosted customers running real (non-mkcert) certs — covered when 2.7.3 (Helm chart + docker-compose bundle) lands. Effort: S (≤½d) for (a) + (b); (c) lives in 2.7.3. |
