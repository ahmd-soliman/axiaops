...
| 2.7.5a | **SaaS entitlement — Phase 1 + 2A scaffold + 2B license-removal** | 🟡 partial | Per `docs/saas-platform-admin-design.md` §7 + ADR-0002 (**Proposed**, sequencing per §7.1). **Phase 1 (platform license)**: runbook in `docs/license-issuance.md` to mint a long-lived (`-days=36500`) `customer_id=axiaops-saas-platform` JWT with the production signing key + inject as `AXIAOPS_LICENSE` — keeps the existing license scan-gate happy with zero gate code. **Phase 2A scaffold** (MR !328 → `288956d`): migration `033_entitlements` (system-scoped, no RLS, `axiaops_runtime`-only); `model.Entitlement` + `EntitlementStatus`; `services/shared/entitlement/` package (pure `IsScanAllowed` predicate, `Resolver`, **fail-closed** `IsScanAllowedForOrg`, `ApplyBillingEvent` billing seam stub); `storage.EntitlementStore` + postgres impl on the admin pool; `cmd/entitlement-seed` CLI. **Phase 2B — license removal, INVERTED to SaaS-default** (branch `feat/saas-license-removal-2b`, commit `2e1069e`, founder-directed): the 4 scan gates (api `scanAccount` + ingestion `scanHandler`/worker/`scanScheduledAccounts`) route through a `gateAllowsScan` helper — nil resolver → license gate; non-nil → per-tenant entitlement gate. **The build-tag model was inverted so SaaS is the DEFAULT build and self-hosted licensed is the opt-in** (an earlier slice made it a `saashosted` opt-in tag + a dev-1-only `-saashosted` deploy variant; both were reverted in `2e1069e`). Default build (no tag, `services/{api,ingestion}/cmd/saasmode_saas.go`, `//go:build !selfhosted`) calls `license.SetEnforcementBypass()` at boot + supplies the store as resolver → entitlement gate; `/v1/version` collapses to `state:"managed"`; dashboard hides the License tab/banner (Settings.jsx + LicenseBanner). `-tags selfhosted` (`saasmode_selfhosted.go`) is the opt-in licensed customer build (nil resolver → license gate); `production` stays the orthogonal DEV_MODE strip. `ENTITLEMENT_GRACE_DAYS` (default 21). Build: `make build-selfhosted` + two **manual** build-stage jobs — `build:selfhosted-shape` (compile+test pin) and `build:images-selfhosted` (`-tags "production selfhosted"`, `-selfhosted`-suffixed image, shipped pre-built — that pre-built distribution is what makes inverting the compile default safe). Both `main.go` log `license: mode resolved mode=saas|selfhosted` at boot. **Every deployed env runs the default SaaS image**; no env deploys the licensed build. **Auto-entitle on org creation**: since the fail-closed entitlement gate now governs every env and billing is unbuilt, migration `034_entitlement_internal_plan` adds an `'internal'` plan + backfills existing orgs, and `Store.ensureDefaultEntitlement` writes an `active`/`internal` row (idempotent, `ON CONFLICT DO NOTHING`, `axiaops_runtime` pool) at the 3 org-create chokepoints (`UpsertOrganization`, `EnsureOrganization`, `ConsumeBootstrapState`). Tests: entitlement-gate handler test (7 statuses), managed-version test, `TestScanScheduledAccounts_SaaS_PerOrgEntitlement`, `entitlement_autocreate_test.go`. **Follow-up #131** (plan/usage page replacing the hidden License page). Decisions: entitlement-table system-scoped, missing-row = deny (fail-closed), platform license prod-key-minted in CI. |
...
| 2.7.11 | **Password breach-corpus check (offline embedded HIBP)** | ✅ | The B1 native-auth password policy was length-only (≥12 chars) — the one outright NIST SP 800-63B §5.1.1.2 violation. **Shipped** the offline-embedded design (`docs/password-breach-check-design.md`): a sorted 20-byte-record SHA-1 blob (`services/api/internal/breachlist/breached-passwords.bin`, `//go:embed`, unconditional in every build including `-tags production`), binary-search membership, **hard-block on hit** (deterministic — no live `api.pwnedpasswords.com` call, works air-gapped). Wired into `auth.CheckPolicy` (length→breach; the 2 CLIs + reset-redeem path inherit the breach screen) + new `auth.CheckPolicyWithIdentity` (length→GitLab-style email/name self-similarity→breach) at the 4 identity-bearing sites (bootstrap, invite-redeem new-user, staff-create; reset-redeem stays plain per v1). `ErrPasswordBreached`/`ErrPasswordContainsIdentity` sentinels flow through the existing `weak_password` 400. SHA-1 is the corpus **index only** — storage stays argon2id. Ingestion does NOT link the blob (structural — `breachlist` is under `services/api/internal`). **Shipped corpus: ~10,044 unique digests (~196 KB), the xato-net top-10,000 merged into a curated seed** — NOT the full HIBP top-1M, a deliberate decision (the 20 MB ordered file is ~0.2 MB binary growth for NIST-sufficient coverage; full-swap remains a one-step regeneration via `scripts/gen-breachlist.sh hibp …` / `cmd/breachlist-gen`). Decision recorded in `docs/password-breach-check-design.md` §Decision. **Licence:** root `NOTICE` credits xato-net/SecLists (MIT, the embedded source) and HIBP / Troy Hunt (CC BY 4.0, scoped to a future full-swap) — repo's first third-party-notice file; no downloader tool is embedded. Provenance + swap procedure: `docs/breachlist-provenance.md`. |
