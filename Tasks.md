...
| 2.7.17 | **[POST-B2] Per-connection `force_reauth` override for SSO `prompt=login`** | 🔲 | Commit `9f30ad8` set `prompt=login` unconditionally on the OIDC authorize URL to close a silent-identity-substitution bug (logged-out user types different email → IdP cookie silently re-auths previous user). The right shape long-term is `prompt=login` as the *default* but with a per-connection `force_reauth: bool` flag so deployments where the IdP enforces conditional access / step-up MFA / "remember me" can opt out. Particularly relevant for Entra customers — Azure AD returns `interaction_required` if a conditional access policy locks session re-auth, which our hardcoded `prompt=login` would conflict with. Surfaces as a support issue when we hit the first Entra customer at scale. Shape: add `force_reauth bool DEFAULT true` column on `sso_connections`, expose in connection CRUD UI, branch in `buildAuthorizeURL`. Effort: S (~2–4h). |
