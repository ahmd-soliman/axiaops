...
| 2.7.5a | **SaaS entitlement — Phase 1 + dormant Phase 2A scaffold** | 🟡 partial | Per `docs/saas-platform-admin-design.md` §7 + ADR-0002 (**Proposed**, sequencing per §7.1). Branch `feat/saas-entitlement-scaffold`. **Phase 1 (platform license)**: runbook in `docs/license-issuance.md` to mint a long-lived (`-days=36500`) `customer_id=axiaops-saas-platform` JWT with the production signing key + inject as `AXIAOPS_LICENSE` — keeps the existing license scan-gate happy on the SaaS deploy with zero gate code (no code change; CI/ops mint). **Phase 2A (DORMANT scaffold)**: migration `033_entitlements` (system-scoped, no RLS, `axiaops_runtime`-only — written by AxiaOps, read cross-org/pre-auth); `model.Entitlement` + `EntitlementStatus`; `services/shared/entitlement/` package (pure `IsScanAllowed` predicate, `Resolver`, **fail-closed** `IsScanAllowedForOrg`, `ApplyBillingEvent` billing seam stub — Stripe out of scope); `storage.EntitlementStore` slice + postgres impl on the admin pool; `cmd/entitlement-seed` CLI for manual/dev row writes. **No scan gate consults entitlement yet** — fully inert, can't regress any deploy. **Phase 2B deferred** (the `cmd/api-saashosted` + `cmd/ingestion-saashosted` roots that call `license.SetEnforcementBypass()` and rewrite the 4 scan gates to consult entitlement; the per-org scheduler restructure; `/v1/version` `state:"managed"`) until ADR-0002 is accepted and the self-serve activation gate proves out (§7.1 / §11.1 #3). Decisions on record: dev-key sample for now (prod-key mint lands in CI), entitlement-table system-scoped, missing-row = deny. |
