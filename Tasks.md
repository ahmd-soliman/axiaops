...
| 2.7.15 | **[POST-B1] Operator smoke-test runbook** | 🔲 | Plan §4.6 AC1, AC4. Short doc at `docs/install-selfhosted.md` (or merge into existing) that walks an operator through: `make start-dev` with `AUTH_PROVIDER=native` from a clean DB → bootstrap → dashboard → invite a member → password-reset round-trip. Plus the `make start-staging` smoke that verifies `AUTH_PROVIDER=kinde` continues to work unchanged during the strangler window. Each step expects a specific UI state; failure modes documented inline (cookie not set → CORS_ORIGIN check; bootstrap 409 → token already consumed). Effort: S (≤1d). |
