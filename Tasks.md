...
_Single source of truth for project work. Last updated: 2026-05-16._
...
**Still pending**: deploy:preview manual click + `/v1/version` smoke against the deployed preview env (operator step); merge MR !85 → `develop` (operator step); C (SAML SP — plan §6). |
| 2.7.3 | **Self-hosted bundle (docker-compose)** | 🔲 | `deploy/compose/docker-compose.selfhosted.yml` for single-VM / docker-host customers — the dominant self-hosted shape (regulated mid-market, EU public sector, on-prem ops; FinOps tooling buyers don't typically run third-party tools inside their K8s estate). Publishes via existing GitLab CI as OCI images. Postgres + Redis included as services with override paths for customer-supplied DBs (so a customer can point at a managed Postgres / Redis if they already operate one). On-host install runbook: `docs/install-selfhosted.md`. Effort: S (~1w). **Helm chart deferred** — the original scope included `deploy/helm/axiaops/`, removed 2026-05-16 (this row's edit) on cost/benefit grounds: building a chart speculatively for a 5-container workload before a prospect has asked for it means ongoing values.yaml / upgrade-hook maintenance against guessed requirements. Re-open as a new row when **a real customer asks in writing with a buying motion attached** — by then the values.yaml shape is informed by that conversation rather than guessed. |
| 2.7.4 | **Self-hosted release versioning policy** | 🟡 partial | Tag scheme + release-notes process + promotion flow shipped via the recent five-MR release-infra series merged into `develop` (commits `b56321e` versioning convention → `b949253` develop→main promotion → `fa59cf5` tag pipelines enabled + production gated to semver → `a3d347a` demo env wired tag-only → `f1ceb7e` CHANGELOG bootstrap → `f709a09` CHANGELOG fixes → `2267872` link-rotation runbook). Lives in [`docs/versioning.md`](docs/versioning.md) (not `docs/release-policy.md` as originally scoped — naming difference, scope superset) + [`CHANGELOG.md`](CHANGELOG.md) (Keep a Changelog 1.1.0). Tag scheme is bare `X.Y.Z` (no `v` prefix — CI substitutes `APP_VERSION=$CI_COMMIT_TAG` verbatim). Pre-1.0 regime + lifecycle suffixes (`-alpha.N` / `-beta.N` / `-rc.N`) documented. **Still pending** before flipping ✅: (1) **release cadence** — pick e.g. "quarterly minor, monthly patch on the active line"; (2) **supported-versions window** — e.g. "last 2 minors get patch fixes"; (3) **migration backwards-compat policy** — what's the contract when a customer skips a minor? Add as new subsection(s) to `docs/versioning.md`. Effort: ≤1d once decided. |
