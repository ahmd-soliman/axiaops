...
| 2.7.11 | **Password breach-corpus check (HIBP k-anonymity)** | 🔲 | The B1 native-auth password policy is currently length-only (≥12 chars). The plan §4.5 D6 mentioned "top-1000 common passwords list" but a static blocklist of any practical size covers <0.1% of the breach corpus and is trivially bypassed (`password1234` blocked → user types `password12345`). Replace with HaveIBeenPwned k-anonymity API: SHA-1(password)[:5] sent to `api.pwnedpasswords.com/range/`, returns hash suffixes, check locally — never sends the password. Surface as an _async_ warning during signup ("this password has appeared in N known breaches; use anyway?") rather than a hard block, so a HIBP outage doesn't block account creation. Lands after rate-limiting (Phase B1 slice 10) and per-account lockout (deferred). Effort: ~½w including tests + the offline-fallback handling. |
| 2.7.12 | **[POST-B1] Re-evaluate DEV_MODE auth bypass** | 🔲 | DEV_MODE was introduced because Kinde JWT validation needed network access to Kinde's JWKS endpoint and a real tenant — unworkable for local dev. With native auth, that justification is gone: bootstrap is one form submission against a fresh DB, no external service. Plan §2 D9 deliberately preserved DEV_MODE for B1 to avoid concurrent churn while the strangler refactor was in flight. **Trigger**: B1 acceptance-complete (MR !68 merged to `feat/sso`). At that point native is provably the dev-friendly path, and the DEV_MODE branch can be evaluated cleanly. **Decision (not work)**: weigh three options against actual operator feedback at that time: (a) **drop entirely** — operators run bootstrap once on fresh-DB dev like everyone else; pro: -30 LoC of auth-bypass branch + removes the "DEV_MODE accidentally set in prod" attack surface; con: every DB wipe costs one extra step. (b) **Replace with `make seed-bootstrap`** — make target that runs bootstrap via curl against the API; pro: removes auth-bypass code path entirely; keeps one-command dev iteration; con: makefile coupling, the curl call needs to know the install token path. (c) **Auto-bootstrap on fresh DB when `DEV_USER_EMAIL` is set** — server detects no-orgs-AND-dev-env and bootstraps itself; pro: keeps current one-step ergonomics; con: keeps an auth-bypass code path, just relabelled. Note: under `AUTH_PROVIDER=kinde\|both` the Kinde JWT validation still happens at runtime — DEV_MODE removal means kinde-mode dev needs a real Kinde tenant or the stub. Acceptable since native is the default by then. Effort once decided: S (≤1d for any of the three). |
...
| 2.7.15 | **[partial — extend] Operator smoke-test runbook** | 🟡 | Plan §4.6 AC1, AC4. **Done in B1**: `docs/native-auth-bootstrap.md` covers the local `make start-staging` flow end-to-end — install-token retrieval (with the zsh-`%`-not-part-of-token gotcha), bootstrap form, post-bootstrap verification, and a troubleshooting section for the common failure modes. The local stack runs plain HTTP (TLS termination is the edge proxy's job in real deployments); cookie posture is correct because the api's helper reads `X-Forwarded-Proto` from the propagated header. **Still to do post-B1**: (a) the same walkthrough but for the legacy `AUTH_PROVIDER=kinde` path during the strangler window, (b) member invite / password-reset round-trip walkthroughs, (c) the install path for self-hosted customers including edge-proxy guidance (the customer's nginx-with-real-certs role) — covered when 2.7.3 (Helm chart + docker-compose bundle) lands. Effort: S (≤½d) for (a) + (b); (c) lives in 2.7.3. |
| 2.7.16 | **[POST-B2] First-run auto-redirect to `/bootstrap`** | 🔲 | Today, a fresh self-hosted install lands on `/login` — a dead-end before any owner exists, since there's nothing to log in *to*. Newcomers have to know that `/bootstrap` exists (the install-token printout names it, but only operators reading container logs see that). Industry convention (Mattermost / Sentry self-hosted / GitLab self-hosted / Bitwarden) is to auto-route to the setup screen on first run, then transition to login after the first owner is bootstrapped. **Shape**: (a) new pre-auth endpoint `GET /v1/auth/bootstrap/state` → `{available: bool}` reading the same `bootstrap_state` row the existing `/v1/auth/bootstrap` POST consults; (b) dashboard mount-time check in `App.jsx`'s initial `useEffect`, before deciding `<Login>` vs `/`, calls the endpoint and `navigate('/bootstrap', {replace: true})` when available + no session cookie; (c) edge cases: cookie present → straight to `/`; endpoint errors → fall through to `/login` rather than blocking; user on `/bootstrap` after consume → redirect to `/login` with a friendly hint. Not a new oracle — anyone can already probe by POSTing junk to `/v1/auth/bootstrap` and reading 409 vs 401. Effort: S (~30–40min including a probe step in `docs/native-auth-bootstrap.md`). Park behind the `feat/sso → develop` merge so the MR scope stays "SSO Phase B + invitation polish". |
...
| 2.7.18 | **[BEFORE-FIRST-PAYING-CUSTOMER] Lock down `/metrics` endpoint** | 🔲 | `services/api/CLAUDE.md` line 22 marks `/metrics` as "internal only" but the dashboard's nginx (`services/dashboard/nginx.conf`) forwards `/api/*` to the API container indiscriminately, so `https://<host>/api/metrics` is publicly scrapable on every deployed env. On AxiaOps's own preview env (axiaops-preview.example.com) this only leaks our own info — caught during the MR !85 metrics-merge fix probe — but the SAME code shipped to a self-hosted customer leaks THEIR `customer_id` (e.g. `axiaops_license_state_info{customer_id="acme-corp",state="valid"}`), license expiry timing (`license_expires_at_seconds`, `license_days_remaining`), zombie counts per organization (`axiaops_zombies_detected{organization_id=...}`), and per-route request rates. **Shape**: in `services/dashboard/nginx.conf`, add `location = /api/metrics { return 404; }` before the `location /api/ { proxy_pass ... }` block — drops external scrapes while internal Prometheus running in the same compose network can still hit `api:8080/metrics` directly via container DNS (the standard self-hosted-app pattern; same as GitLab CE, Mattermost, Sentry self-hosted). Couples with a one-line note in `docs/license-issuance.md` operator runbook explaining how to scrape internally. **Effort**: XS (~15min including the regression test that curls `/api/metrics` against the running staging stack and asserts 404). Same gate as B1.7 layer 4 (issue #75): "before first paying self-hosted customer ship". Not a !85 blocker — internal preview leak is just AxiaOps data. |
| 2.7.19 | **[POST-B2] Pass `login_hint=<email>` on OIDC `/initiate`** | 🔲 | Surfaced during the `docs/sso-local-keycloak.md` Probe 11 walkthrough on 2026-05-06. Today the email-blur SSO discovery flow ends with the api 302'ing the browser to the IdP authorize URL carrying `prompt=login` (forces fresh auth — pinned by `services/api/internal/sso/initiate_test.go`) but NOT `login_hint`. Result: Keycloak/Entra/Okta render their login form with an empty username field — the user types their email at /login, then types the same email again at the IdP. Friction without a security justification. RFC 6749 §3.1.2 + OIDC Core §3.1.2.1 specify the `login_hint` parameter exactly for this case: the RP gives the IdP a strong "this email is the one to authenticate" hint while the IdP still controls the actual auth UX. **Shape**: in `services/api/internal/sso/initiate.go`'s `buildAuthorizeURL`, append `login_hint=<email>` from the discover-context-bound email when present. The email is already in scope at /initiate (the discover hop persisted it into state). Test pin in `initiate_test.go` matching the existing `prompt=login` shape — assert the parameter is on the URL, URL-encoded if it contains `+`/`@`. **Effort**: XS (~10min code + 1 test). Park behind `feat/sso → develop` merge so the MR scope stays "SSO Phase B + invitation polish". Not a security improvement; pure UX polish. |
| 2.7.20 | **[POST-B2] Annotate invitation `redemption_url` when SSO enforcement=required** | 🔲 | Surfaced during the `docs/sso-local-keycloak.md` Probe 7 walkthrough on 2026-05-06. When an admin issues an invitation for an email on an SSO-verified domain in an org with `enforcement='required'`, the api emits a `redemption_url` (the native-password redemption path) that's a UX footgun: invitee clicks it, sets a password via `POST /v1/auth/invitations/redeem`, gets a session with `auth_mode='password'`, then EVERY subsequent authed request 403s with `sso_required` because the `EnforceSSO` middleware blocks password sessions in enforcement=required orgs. The URL "works" for the redemption hop then immediately bricks the user. **The token still needs to be generated** (cross-org B1.5 invitees on non-SSO domains, enforcement-flips mid-flight, IdP-outage break-glass) — the fix is informational, not structural. **Shape**: (a) backend `POST /v1/invitations` response shape gains an optional `enforcement_hint` field set to `"sso_required"` when the inviter's org has `enforcement='required'` AND the invitee's email domain is verified for that connection — copy of the resolved enforcement state at invite-creation time, not a live join. (b) Dashboard's create-invite success modal renders a yellow callout when `enforcement_hint='sso_required'`: "SSO is enforced for this org — the invitee will be auto-onboarded on first SSO login. This URL is a break-glass and may not work for the invitee depending on their auth provider." with a copy button on the URL itself for the break-glass case. (c) `services/api/internal/api/invitations_test.go` pin: assert the field is present in the response shape only when both conditions hold; absent in optional/preferred/empty enforcement orgs. **Effort**: S (~2–3h including the dashboard copy + test). Park behind `feat/sso → develop` merge. Not a security improvement; pure UX clarity for the SSO-enforced flow. |
...
| 14 | **Email-based team invitations (~~Kinde Mgmt API~~ → native, per [ADR-0001](docs/decisions/0001-deployment-model.md))** | 🔲 | _Originally designed against Kinde's Management API (see `docs/invitation-flow.md`); **must be re-designed for native auth** under ADR-0001._ The `pending_memberships` table + redemption-on-first-login flow is preserved as the data shape — only the email-delivery and auth-redemption paths change. New design: AxiaOps directly sends invitation emails via SMTP/Resend (need to pick a provider — track as part of Self-hosted v1, see below); invitation token validates against `pending_memberships.invite_token` on `/auth/accept-invite/{token}`; on signup or login that hits a verified token, a `memberships` row is written. Update `docs/invitation-flow.md` accordingly. Estimated ~10 days (was ~7.75 days under Kinde). |
